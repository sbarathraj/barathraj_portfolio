      ...
      >
        <div className={`container mx-auto flex items-center justify-between px-0 md:px-2 transition-all duration-300 ${isScrolled ? 'h-12 md:h-14' : 'h-16 md:h-20'}`}>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <motion.button
              ...
              className={`md:hidden text-gray-600 ${isScrolled ? 'p-1' : 'p-2'}`}
        ...
        {isMobileMenuOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-40 md:hidden bg-white/20 backdrop-blur-lg border-t border-white/20"
...
const AnimatedMobileMenu: React.FC<AnimatedMobileMenuProps> = ({
  ...
  return (
    <motion.div
      ...
      className="fixed top-0 right-0 bottom-0 w-4/5 max-w-xs z-50 flex flex-col p-6 md:hidden bg-white/20 backdrop-blur-lg border-l border-white/20 rounded-l-2xl shadow-2xl"
    ...
    >
      ...
      <div className="flex items-center justify-between mb-8">
        <motion.button
          ...
          className="text-gray-600 p-2 rounded-full hover:bg-gray-200 transition-colors"
          onClick={onClose}
      ...
      <div className="flex flex-col justify-between h-full">
        ...
        <div className="mt-auto pt-8 border-t border-white/20">
          <h3 className="text-sm font-medium text-gray-600 mb-4">Connect with me</h3>
          <div className="grid grid-cols-3 gap-4">
            {socialLinks.map(({ icon: Icon, href, label, color }) => (
              <motion.a
                ...
                whileTap={{ scale: 0.9 }}
                className={`flex flex-col items-center gap-2 p-3 rounded-xl bg-white/10 hover:bg-white/20 transition-all ${color}`}
                aria-label={label}
              >
                <Icon size={24} />
